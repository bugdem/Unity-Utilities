...
namespace ClocknestGames.Library.Control
{
	...
	public class CGTouchFollowingJoystick : CGTouchJoystick, EventListener<TouchEvent>
	{
		...
		public float JoystickPercentage;
		...
		protected override void Start()
		{
			...
			if (_maxRangeObject != null)
				MaxRange = Vector3.Distance(transform.position, _maxRangeObject.transform.position);
		}
		...
		protected override void Update()
		{
			...
			if (Vector2.Distance(_newTargetPosition, parentPosition) > MaxRange)
				_parentRectTransform.transform.position = _newTargetPosition + (parentPosition - _newTargetPosition).normalized * MaxRange;
			...
			JoystickPercentage = GetJoystickPercentage();
		...
		/// <summary>
		/// Returns Knob position percentage in MaxRange.
		/// </summary>
		/// <returns></returns>
		public float GetJoystickPercentage()
		{
			float distance = Vector2.Distance(transform.position, _parentRectTransform.transform.position);
			return CGMaths.Remap(distance, 0f, MaxRange, 0f, 1f);
		}
