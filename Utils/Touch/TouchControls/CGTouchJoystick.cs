...
namespace ClocknestGames.Library.Control
{
    ...
    public class CGTouchJoystick : MonoBehaviour, IDragHandler, IEndDragHandler
    {
        ...
        public JoystickEvent JoystickValueEvent;

        [SerializeField] protected Vector2 _joystickValue;
        ...
        {
            if (JoystickValueEvent != null)
            {
                if (HorizontalAxisEnabled || VerticalAxisEnabled)
                {
                    JoystickValueEvent.Invoke(_joystickValue);
                }
            }
        }
